...
if [[ "${driver}" != "zfs" ]]; then
    echo -e "${ERROR}: wrong backend ${driver}"
    EXITCODE=23
else
    echo "OKAY."
fi
...
if [[ "${NUM_ZPOOLS}" -eq "1" ]]; then
    ...
    echo -e "${ERROR}: one expected but got ${NUM_ZPOOLS}"
    EXITCODE=23
fi
...
if [[ "${ZPOOL_COUNT}" -eq "${DEVICES_COUNT}" ]]; then
    ...
    echo -e "${ERROR}: ${ZPOOL_COUNT} of ${DEVICES_COUNT} in use."
    EXITCODE=23
fi
...
if [[ ${EXITCODE} -eq "0" ]]; then
    ...
    echo -e "${ERROR}: RUNNING TESTS ON ${TESTNAME}!"
fi
exit ${EXITCODE}